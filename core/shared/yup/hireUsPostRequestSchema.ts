...
const budgetTitles = HIRE_US_BUDGET_OPTIONS.map(x => x.label);

const hireUsPostRequestSchema = object({
  firstName: string()
  ...
  companyName: string()
    ...
    .required(),
  budget: string()
    .oneOf(budgetValues, `Budget must be one of ${budgetTitles.join(", ")}`)
    .required(),
  projectInfo: string()
...
export default hireUsPostRequestSchema;
